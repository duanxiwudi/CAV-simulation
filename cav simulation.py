...
parser.add_argument('--path_of_network', default = os.path.join(currentDir, "network"), help = "the directory of network file, including .inpx and .layx")
...
parser.add_argument('--duration', default = 3600 ,type = int, help = "simulation duration, unit in second")
...
#output
    ...
    dataSet_signal.to_csv(simulation.path_output_file +'\\Signal_data.csv',  index=False )    
...
## data_vehicles are for AV/CV modeling purpose        
        ...
        dist_stop = (Desired_speed / 3.6) **2 / 2 / -(simulation.comf_dec)
...
# since the AV logic does not consider signal, we designed a signal controlled mode that vehicles are going to decelerate at comfortable deceleration rate when get close enough to signal head.
            ...
            if (vehicle[8] == 'VEHICLE' and leading(vehicle[7],data_vehicles)):
                ...
                ak = min(simulation.comf_acc, (Desired_speed  - vehicle[4]) / 3.6 *0.58 if vehicle[4] < Desired_speed else 0 )
